         ...
         {! post?.hasOwnProperty("title")  ? (
          <div className="py-1 px-2">
            <p className="font-sans text-lg">{post?.content?.body}</p>
             <div className="bg-gray-400 w-[100%] xs:h-[310px]  md:h-[326px] lg:h-[386px] border-red-600 border rounded-md ">
                {post?.content?.media?.map((img, i) => {
                  const imgUrl = img.url.replace("ipfs://", `${img.gateway}`)
                  return(
                  <img key={i} src={imgUrl} alt='picture ' 
                    className="w-[100%] xs:h-[310px] max-h-[100%] md:h-[360px] lg:h-[386px] object-cover rounded-md "
                  />
                  )
                })}
             </div>
